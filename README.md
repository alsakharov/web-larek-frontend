...
### 3. Слой Презентера (`src/components`)
...
```
[ProductListView] <---> [ProductListPresenter] <---> [Catalog]
      |                        |                        |
[CartView] <------> [CartPresenter] <------> [Cart]
      |                        |                        |
[OrderFormView] <--> [OrderPresenter] <--> [FormModel]
      |
[ContactFormView]
      |
[ModalView] <--> [SuccessModalView], [ErrorView]
```
