...
function openProductModal(productId: string) {
  ...
  const addBtn = productModal.getElement().querySelector('#add-to-cart') as HTMLButtonElement;
...
function setupModalCloseHandlers() {
  document.addEventListener('click', (event) => {
    ...
    if (target.classList.contains('modal') && target.classList.contains('modal_active')) {
      closeAllModalsWithScroll();
    }
  });
}
